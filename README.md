...
2. Compute the similarity matrix \( S \in \mathbb{R}^{k \times k} \) where each entry \( S_{ij} = S(a_i, b_j) \) represents the similarity between token \( a_i \in A \) and \( b_j \in B \).

3. Select the top \( r \) pairs based on similarity, i.e., find the edges \( \{ (a_i, b_j) \}_{i,j} \) with the largest \( S_{ij} \) values, where \( r \) is the desired number of token mergers.
